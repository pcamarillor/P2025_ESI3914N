...
how often logging
how many messages per logging
which information type of logging
which information structure       
...
class Producers:
    ...
    def __init__(self, ticker: string, kafka_server: string, kafka_topic: string, log_time: float = 2.0, start_date: string = "2025-01-01", end_date: str = None):
        '''
        ...
        Args:
            log_time (int, optional):               int or 'random', interval between log messages, default is 2 seconds
            log_file_number (int, optional):        number of log messages pero log file, default is random 
            log_topic (string):                     which topic to stream ()
        Returns:
            
        '''
        
        
        self.run_logs       = False       #bool for active logging      
    ...
    def __resample_and_aggregate(self, df ,new_window):
        """
        produce prices on interval of 5secs
        calculate ohcl every 5mins
        so in interval of 60/5*5 = 60 elements
        ...
        ohlc_df['price'] = pd.to_numeric(ohlc_df['price'], errors='coerce')
        ...
        ohlc_df['open']     = ohlc_df['price'].resample(new_window).first()
        ohlc_df['high']     = ohlc_df['price'].resample(new_window).max()
        ohlc_df['low']      = ohlc_df['price'].resample(new_window).min()
        ohlc_df['close']    = ohlc_df['price'].resample(new_window).last()
        ohlc_df             = ohlc_df.dropna()
    ...
    def __start_producer(self):
        ...
        self.last = self.hist["Close"].iloc[-1, 0]  
        while self.run_logs:
            ...
            try:
                ...
                self.last                       = next_prices_trajectory["Close"].iloc[-1, 0]  
